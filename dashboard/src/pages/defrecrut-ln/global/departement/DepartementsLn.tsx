...
import { departementsLnList } from '../../../../services/providers/store/atoms/departement.atom';
import AddDepartementLn from './AddDepartementLn';

function LoadDepartement({ reload }: { reload: any }) {
  const { id } = useParams();
  const departements = useRecoilValue(departementsLnList(id ?? ''));
  ...
  const removeItems = async (rows: any[]) => {
    await axios.post(
      config.api_url.defrecrutLn + 'departements/archive',
      rows.map((row) => row.id)
    );
    reload();
  };
...
function DepartementsLn() {
  ...
  return (
    <div>
      <div className="fs-20 bold">Départements</div>
